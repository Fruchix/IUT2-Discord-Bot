...
def loadEdt(resources, firstDate, lastDate):
    ...
    :param firstDate: le jour de début de semaine
    :type firstDate: str
    :param lastDate: le jour de fin de semaine (semaine de 5 jours de travail)
    :type lastDate: str
    ...
    # requête récupérant le calendrier iCal, passage du dictionnaire paramRequete en paramètres de l'URL
    r = requests.get("https://ade-uga.grenet.fr/jsp/custom/modules/plannings/anonymous_cal.jsp?projectId=1&calType=ical", params=paramRequete)
    # récupération du contenu du calendrier iCal
    edtCal = Calendar(r.text)
    ...
    for event in list(edtCal.events):
...
def afficheListeCours(liste):
    """Afficher les cours d'une liste de cours.
...
maListe = loadEdt(idEdtGroupe['A2'],"2022-03-21","2022-03-25")
afficheListeCours(maListe)